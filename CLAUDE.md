...
|-------|--------|------------|
| 2026-04-28 | Tarjetas de producto en POS más compactas + íconos personalizados por producto: rediseño de tarjetas (30% más pequeñas), selector visual de íconos RemixIcon en Create/Edit, campo `Icono` en base de datos | `Models/Entities/Producto.cs` (campo Icono), `Migrations/20260428235253_AddIconoProducto.cs`, `Views/POS/Index.cshtml` (usa ícono personalizado), `Views/Productos/Create.cshtml` (selector de íconos), `Views/Productos/Edit.cshtml` (selector de íconos), `Views/Productos/Details.cshtml` (muestra ícono), `wwwroot/css/pos-limpio.css` (tarjetas compactas) |
| 2026-04-27 | Módulo de Productos mejorado: precio de compra, cálculo de ganancia en tiempo real, tipo de producto (Artículo/Servicio/Libra), campos extendidos (lote, vencimiento, ubicación, proveedor, stock mínimo, peso por unidad, unidad de medida) | `Models/Entities/Producto.cs` (11 nuevos campos), `Models/Entities/Proveedor.cs` (navegación Productos), `Data/ApplicationDbContext.cs` (configuración relación Proveedor), `Migrations/20260427000002_AddProductoExtended.cs` (nueva migración), `Controllers/ProductosController.cs` (actualizado Create/Edit con todos los campos), `Views/Productos/Create.cshtml` (rediseño completo 7 secciones), `Views/Productos/Edit.cshtml` (rediseño completo), `Views/Productos/Details.cshtml` (actualizado con nueva información) |
| 2026-04-27 | Rediseño completo del módulo POS con UI profesional, carrito dinámico sin recargas, modal de pago múltiple, atajos de teclado y vista de éxito mejorada | `Controllers/POSController.cs` (nuevas acciones: FiltrarProductos, GetCategorias, VentasDelDia, ProcesarPago), `Views/POS/Index.cshtml` (rediseño completo), `Views/POS/Completada.cshtml` (rediseño completo) |
